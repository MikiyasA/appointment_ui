            ...
            try {
                ...
                if (userType === "client") {
                    ...
                    const res = await fetch(`${process.env.NEXT_PUBLIC_BE_BASE_URL}/clients`, {
                        ...
                        body: JSON.stringify({ email: user.email, firstName, lastName, status: "Auto-Added", role: ["client"] }),
                    });
                    ...
                    const res = await fetch(`${process.env.NEXT_PUBLIC_BE_BASE_URL}/staff`, {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ email: user.email, firstName, lastName, status: "Auto-Added", role: ["New-Staff"] }),
                    });
                    const result = await res.json();
